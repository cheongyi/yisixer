...
-define(DEFAULT_STANDALONE_TIMEOUT, 36000000).
...
init(Host, Port, User, Password, Database, LogFun, Parent) ->
	...
	{ok, RecvPid, Sock} ->
		...
		{ok, Version} ->
		    case do_query(Sock, RecvPid, LogFun, " set names utf8", Version, [{result_type, binary}]) of
			{error, MySQLRes} ->
			...
			{_ResultType, _MySQLRes} ->
                do_query(Sock, RecvPid, LogFun, "use " ++ Database, Version, [{result_type, binary}]),
                do_query(Sock, RecvPid, LogFun, "SET FOREIGN_KEY_CHECKS=0", Version, [{result_type, binary}]),
...
loop(State) ->
	...
	{fetch, Ref, Query, GenSrvFrom, Options} ->
	    ...
	    loop(State);
	{mysql_recv, RecvPid, data, Packet, Num} ->
	    mysql:log(State#state.log_fun, info, "mysql_conn: Received MySQL data when not expecting any "
		      "(num ~p) - ignoring it", [Num]),
	    mysql:log(State#state.log_fun, info, "mysql_conn: Unexpected MySQL data (num ~p) :~n~p",
		      [Num, Packet]),
	    loop(State);
	close ->
