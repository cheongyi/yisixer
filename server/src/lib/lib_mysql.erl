...
%%% ========== ======================================== ====================
insert (PoolId, Sql) ->
    case game_mysql:fetch(PoolId, Sql) of
        {updated, Result} ->
...
%%           MysqlResult= mysql_result(),    game_mysql:fetch/3，返回的结果集
...
readble_column (N, Row, FieldInfo) ->
    ...
    ReturnValue = if
        Value == null->
            null;
        FieldType == 'LONG'     orelse  %% INT
        FieldType == 'LONGLONG' orelse  %% INT
        FieldType == 'TINY'     orelse  %% TINYINT
        FieldType == 'SHORT'    orelse  %% SMALLINT
...
%% Example  :
%% (server_00@localhost)20> lib_game_mysql:get_data(mysql_pool, "test", "idtest = 1", "*").
...
update_row (PoolId, Table, Key, Values) ->
    ...
    case game_mysql:fetch(PoolId, [Sql]) of
        {updated, Result} ->
            game_mysql:get_result_affected_rows(Result);
        _ ->
            false
    end.
