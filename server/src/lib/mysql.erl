...
start_link() ->
    LogFun = fun(Level, Message, Arguments) -> 
		case Level of 
			error -> ?ERROR(Message ++ "~n", Arguments);
			_     -> ok
		end
	end,
    start_link(gamedb, ?MYSQL_HOST, ?MYSQL_PORT, ?MYSQL_USERNAME, ?MYSQL_PASSWORD, ?MYSQL_DATABASE, ?MYSQL_POLLSIZE, LogFun, false).
...
start_link(Id, Host, Port, User, Password, Database, PoolSize, LogFun, Reconnect) 
